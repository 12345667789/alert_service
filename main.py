...
class NotificationSender:
    ...
    def send_telegram_with_retry(
        self, message: str, max_retries: int = 2, delay: float = 1.0
    ) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Telegram failed with status {response.status_code}: {response.text}"
                    )
                    if attempt < max_retries:
...
class ConfigManager:
    ...
    def get_webhook_config(self) -> dict:
        ...
        try:
            ...
            discord_webhook = (
                discord_doc.to_dict().get(discord_field) if discord_doc.exists else None
            )
            telegram_doc = db.collection("app_config").document("telegram_token").get()
            telegram_token = (
                telegram_doc.to_dict().get("API_key") if telegram_doc.exists else None
            )
            telegram_chat_id = (
                telegram_doc.to_dict().get("chat_id") if telegram_doc.exists else None
            )
...
def pubsub_handler():
    ...
    try:
        ...
        if not envelope or "message" not in envelope:
            return (
                jsonify({"status": "error", "message": "Invalid Pub/Sub message"}),
                400,
            )
...
def test_telegram():
    """Test the full Telegram notification flow"""
    try:
        data = request.get_json() or {}

        # Create a sample alert to test formatting and sending
        test_alert = {
            "symbol": data.get("symbol", "TEST"),
            "underlying": data.get("underlying", "Test Company Inc."),
            "alert_type": "new_breaker",
            "trigger_time": "12:00:00",
            "source": "Local Test",
        }

        # Use the same functions that the real handler uses
        telegram_message = notification_sender.format_telegram_message(test_alert)
        result = notification_sender.send_telegram_with_retry(telegram_message)

        if result == "success":
            return jsonify(
                {"status": "success", "message": "Telegram test sent successfully."}
            )
        else:
            return jsonify({"status": "failed", "reason": result}), 500
